...
def load_env_profiles() -> Dict:
    _ensure()
    if not ENV_FILE.exists(): return {}
    try: return json.loads(ENV_FILE.read_text())
    except Exception: return {}

def save_env_profiles(profiles: Dict):
    _ensure()
    ENV_FILE.write_text(json.dumps(profiles, indent=2))

# ── DQ History ────────────────────────────────────────────────
DQ_FILE = DATA_DIR / "dq_history.json"

def save_dq_summary(result) -> None:
    _ensure()
    history = []
    if DQ_FILE.exists():
        try: history = json.loads(DQ_FILE.read_text())
        except Exception: pass
    history.insert(0, {
        ...
        "summary_pass":   result.summary_pass,
        "summary_warn":   result.summary_warn,
        "summary_fail":   result.summary_fail,
        "dropped_cols":   result.total_dropped_cols,
        "type_issues":    result.total_type_issues,
    })
    DQ_FILE.write_text(json.dumps(history[:100], indent=2))

def load_dq_history() -> List[Dict]:
    _ensure()
    if not DQ_FILE.exists(): return []
    try: return json.loads(DQ_FILE.read_text())
    except Exception: return []
